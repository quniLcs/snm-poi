    ...
    for userId_i in tqdm(user_id_list, ncols=80):
        ...
        for userId_j in user_id_list:
            ...
            if mode == "slide_window":
                ...
                temp.append((userId_j, LCS(traj_i, traj_j)))
...
def compute_topk_venue_dict_for_Bk(data_root,
                                   top_k=10):
    
    dataset = BrightKite(data_root, load_geo=False)
    ...
    pos_array = np.zeros((n_venue, 2), dtype=np.float32)   # [N, 2]
    for venue_id in venue_id_list:
        pos_array[venue_id2idx[venue_id], :] = [float(venue_dict[venue_id][0]), float(venue_dict[venue_id][1])]
    ...
    save_path = os.path.join(data_root, "Brightkite_topk_close_venue.pkl")
    with open(save_path, "wb") as f:
        pickle.dump(topk_close_venue_dict, f)       
